...
def main(database_path, video_filename):
    ...
    out = cv2.VideoWriter('SSD_module_wander_low_th_3_max1_5fps_.avi', fourcc, 30.0, (int(width), int(height)))
    frame_num = 0
    for json_file_name in json_file_list:
        ...
        with open(os.path.join(json_folder_path,json_file_name), "r") as json_file:
            ...
            for bbox in bboxs:
                # if bbox["label"][0]["score"] < 50:
                #     print(json_file_name, " - bbox score < 50!! ")
            
                # if bbox["label"][0]["description"] == "person" and  bbox["label"][0]["score"] > 50:
                if bbox["label"][0]["description"] == "person" :
                    box = bbox["position"]
    ...
    json_output_path = "./wander_output_list_30fps_30.json"
